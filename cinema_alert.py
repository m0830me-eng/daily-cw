...
STATE_FILE = "seen_cgv_wangsimni.json"
BASELINE_FILE = "baseline_cgv_wangsimni.done"
BOOKING_STATE_FILE = "cgv_wangsimni_booking_state.json"
BOOKING_STATE_SCHEMA = "CGV_WANGSIMNI_BOOKING_STATE_V1"
...
GV_WEBHOOK = (
    os.environ.get("DISCORD_CGV_WANGSIMNI_GV", "").strip()
    or COMMON_WEBHOOK
)
STAGE_WEBHOOK = (
    os.environ.get("DISCORD_CGV_WANGSIMNI_STAGE", "").strip()
    or COMMON_WEBHOOK
)
DISCORD_USER_ID = (
...
def _send_grouped_alert(group_items, alert_kind):
    ...
    for item in sorted(group_items, key=lambda x: clean_text(x["event"].get("time", ""))):
        lines.append(_alert_line(item["event"]))

    ok = send_discord(webhook_for_type(event_type), "\n".join(lines))
    if ok:
        print(f"🔔 묶음 알림 | {date} | {event_type} | {notification_title(event_type, alert_kind)} | {len(group_items)}회차")
    return ok
...
def _prepare_event_transition(key, event, seen, booking_state):
    ...
    previous = previous_record.get("status", "UNKNOWN") if isinstance(previous_record, dict) else "UNKNOWN"
    is_new_key = key not in seen

    def pending(alert_kind, target_state):
        return {"key": key, "event": event, "alert_kind": alert_kind, "target_state": target_state, "mark_seen": is_new_key}
...
def process_events_grouped(events, seen, booking_state):
    ...
    for key, event in events.items():
        item = _prepare_event_transition(key, event, seen, booking_state)
        if item is not None:
    ...
    for item in pending_items:
        event = item["event"]
        group_key = (event.get("date", ""), event.get("type", ""), item["alert_kind"])
    ...
    for group_key in sorted(groups):
        ...
        for item in items:
            ...
            if item["mark_seen"]:
                seen.add(key)
            booking_state[key] = state_record(event, item["target_state"])
